...
import { IEntityProcedure } from "./standart/procedures/entity/IEntityProcedure.js";
...
export abstract class EntityClass extends EventEmitter implements Bootable {
  ...
  public get schema(): EntitySchema {
    if (this._schema == null) this._schema = this.buildSchema();
  ...
  constructor(name: string) {
    ...
    let columns = this.defineColumnSchema();
    ...
    return new ColumnClass({
      ...
      validators: (value) => {
        ...
        return validStatus.includes(value)
          ? true
          : "'Status' column only accepts " + validStatus.join(" , ");
      },
    });
  }
  ...
  public addReferences(
    ...references: (ReferenceClass | ReferenceClassParameters)[]
  ) {
  ...
  public getBootableName(): string {
    return `BootOfEntity(${this.name})`;
  }

  public getReferences(): ReferenceClass[] {
    let allRefs: ReferenceClass[] = [];
    for (let referenceName in this._references) {
      allRefs.push(this._references[referenceName]);
    }
    return allRefs;
  }

  public async existsInAuria(conn: Knex, info : IConnection): Promise<boolean> {
    return conn
      .select("*")
      .from(SystemEntityCatalog.Entity.table_name)
      .where("name", this.name)
      .where("connection_id", info._id)
      .then((res) => {
  ...
  public async saveIfNotExistsInAuria(conn: Knex, info : IConnection) {

    const notExists = !(await this.existsInAuria(conn, info));

    if (notExists) await this.saveInEntityTable(conn, info);

    for (let columnName in this._columns) {
      let column = this._columns[columnName];
      await column.saveIfNotExistsInAuria(conn, this.name);
    }
  }

  protected async saveInEntityTable(conn: Knex, info : IConnection) {
    let entityInfo: IEntity = {
      connection_id: info._id,
      name: this.name,
      table_name: this.schema.table_name,
      is_system_entity: this.schema.is_system_entity,
      title: this._info.title,
      description: this._info.description ?? "",
      status : 'active'
    };

    return conn.insert(entityInfo).into(SystemEntityCatalog.Entity.table_name);
  }
  ...
  protected abstract defineColumnSchema() : (ColumnClass | ColumnClassParameters)[];
