...
import { IResource } from "../../schemaInterface/IResource.js";
...
export class ResourceSchema extends DefaultSchema<IResource> {
    ...
    public addColumns(...columns: ColumnSchema[]) {
        columns.forEach((column) => {
            ...
            if (this.columns[columnName] != null)
                throw new Error(`[ResourceSchema] Duplicate column on resource! Column with name ${columnName} already exists in ${this.get("name")}`);
    ...
    public addReferences(...references: ReferenceSchema[]) {
        references.forEach((reference) => {
            ...
            if (this.references[referenceName] != null)
                throw new Error(`[ResourceSchema] Duplicate reference on resource! Reference with name ${referenceName} already exists in ${this.get("name")}`);
            ...
            .then((tableExists) => {
                if (tableExists) {
                    ...
                    let tableCreation = connection.schema
                        ...
                        .then((created) => {
                            console.log("[ResourceSchema] Table creation > ", created);
                            return this;
                        });

                    tableCreation.catch((err) => {
                        console.error("[ResourceSchema] SQL Error!", err);
                        throw new Error("[ResourceSchema] Failed to create table! SQL Error");
                    });
